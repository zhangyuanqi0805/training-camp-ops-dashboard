...
const SCREENSHOT = path.join(ROOT, 'screenshots', 'V1_5_5_602默认首页_桌面.png');
const MOBILE_SCREENSHOT = path.join(ROOT, 'screenshots', 'V1_5_5_602默认首页_手机.png');
...
async function main() {
  ...
  try {
    ...
    for (const label of ['每日经营三指标', '当日新增LTV', '选修作业率', '到课率（主课观看）', '总配额占比漏斗', '班级营收前五', '直接问经营问题']) {
      assert(bodyText.includes(label), `missing restored block: ${label}`);
    }
    const trendText = await page.locator('.daily-trend-wrap').last().innerText();
    assert(!trendText.includes('必修作业率'), 'daily trend must not retain the mandatory-homework label');
    ...
    const shares = await page.locator('.quota-funnel-share').allInnerTexts();
    ...
    const geometry = await page.locator('.quota-funnel-shape').evaluateAll((nodes) => nodes.map((node) => ({
      leftTop: parseFloat(node.style.getPropertyValue('--funnel-left-top')),
      rightTop: parseFloat(node.style.getPropertyValue('--funnel-right-top')),
      leftBottom: parseFloat(node.style.getPropertyValue('--funnel-left-bottom')),
      rightBottom: parseFloat(node.style.getPropertyValue('--funnel-right-bottom')),
      clipPath: getComputedStyle(node).clipPath,
    })));
    assert(Math.abs(geometry[0].leftTop) < 0.1 && Math.abs(geometry[0].rightTop - 100) < 0.1,
      'the full funnel must begin with a 100% top edge');
    assert(geometry[geometry.length - 1].rightBottom - geometry[geometry.length - 1].leftBottom >= 44,
      'the final layer must retain a visibly wide lower base');
    geometry.slice(0, -1).forEach((stage, index) => {
      assert(Math.abs(stage.leftBottom - geometry[index + 1].leftTop) < 0.01 &&
        Math.abs(stage.rightBottom - geometry[index + 1].rightTop) < 0.01,
        `adjacent layers must share a continuous straight edge at ${index}`);
    });
    ...
    assert(await page.locator('.daily-operating-trend [data-series="elective-homework-rate"]').count() === 1,
      'elective-homework line must render');
    ...
    const electivePoints = page.locator('circle[data-trend-point="elective-homework-rate"]');
    assert(await electivePoints.count() === 10, '602 elective-homework line must use D1-D10 real values');
    const homeworkPoint = electivePoints.nth(4);
    await homeworkPoint.hover();
    assert((await ltvTooltip.innerText()).includes('选修作业率'), 'elective-homework point must update the custom tooltip');
    assert((await homeworkPoint.getAttribute('fill')) === '#1f8a70', 'elective-homework line must be green');
    const funnelValues = await page.locator('.quota-funnel-value').allInnerTexts();
    const compactFunnelValues = funnelValues.map((value) => value.replace(/\s+/g, ''));
    assert(compactFunnelValues[0] === '加好友999人' && compactFunnelValues[compactFunnelValues.length - 1] === '全款71人',
      'stage labels and counts must sit inside the funnel');
    const yoyValues = await page.locator('.quota-funnel-yoy').allInnerTexts();
    assert(JSON.stringify(yoyValues) === JSON.stringify(['+22.1%', '+22.2%', '+22.0%', '+22.4%', '+22.5%', '+21.8%', '+33.3%', '+42.0%']),
      `602 period-over-period values are wrong: ${JSON.stringify(yoyValues)}`);
    ...
    assert((await page.locator('.quota-funnel-share').first().innerText()) === '97.4%',
  ...
  console.log('V1.5.5 elective-homework funnel browser QA: PASS');
}
